...
<#
.SYNOPSIS
  Builds (optional) and packs Sonic Chest Filters for Thunderstore / r2modman.
...
.PARAMETER SkipBuild
  Package the existing bin\<Configuration>\SonicChestFilters.dll without rebuilding.
...
$Dll = Join-Path $ProjectRoot "bin\$Configuration\SonicChestFilters.dll"
...
$PluginSrc = Join-Path $ProjectRoot "src\SonicChestFilters.cs"
$CsprojPath = Join-Path $ProjectRoot "SonicChestFilters.csproj"
...
Assert-True ($pluginSrcRaw -match 'PluginVersion\s*=\s*"(\d+\.\d+\.\d+)"') `
    "Could not find PluginVersion in SonicChestFilters.cs"
...
Assert-True ($csprojRaw -match '<Version>(\d+\.\d+\.\d+)</Version>') `
    "Could not find <Version> in SonicChestFilters.csproj"
...
Copy-Item -LiteralPath $Dll -Destination (Join-Path $Stage "SonicChestFilters.dll")
...
try {
    ...
    $requiredInZip = @("manifest.json", "README.md", "icon.png", "SonicChestFilters.dll")
...
Copy-Item -LiteralPath $Dll -Destination (Join-Path $OutDir "SonicChestFilters.dll") -Force
...
Write-Host "Or import the zip into r2modman as a local mod (installs as SonicDM-SonicChestFilters)."
